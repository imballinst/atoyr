...
- **AGS Leaderboard**: `LeaderboardService` delegates to extend-leaderboard via `LeaderboardReadClient`. A composite score (`score * 1_000_000 + round(accuracy * 10_000) - durationSeconds`) is the leaderboard point. When extend-leaderboard is not configured, it falls back to a `FakeLeaderboardClient` (in-memory).
