...
- For long blocks (`if`, `for`, etc.), separate the logical steps with blank lines so the flow is easy to follow.
...
- **Dashboard** (`web/static/index.html`): Vanilla HTML with TailwindCSS, Chart.js, and Google Identity Services via CDN. Polls `/admin/stats` every 5s. Shows 4 stat cards + 4 time series charts with period selector (1h/24h/7d/1M).
- **Mode/topic breakdown**: `GET /admin/stats` returns `modeBreakdown` with one row per mode, and each mode has a `topics` array. The session-count queries group by `mode, topic`. **When adding a new topic or mode, update the dashboard**: `StatsService` is constructed with the topic list from `wordService.GetTopics()`, so new topics appear automatically; but (1) add blind-mode-compatible topics to `BlindSupportedTopics` in `stats.service.go` (mirrored by the game routes validation), and (2) add a label for the topic in `TOPIC_LABELS` in `web/static/index.html`.
...
Key files: `packages/server/cmd/main.go`, `packages/server/internal/services/game.service.go`, `packages/server/internal/services/session.service.go`, `packages/server/internal/middleware/metrics.go`, `packages/server/internal/middleware/auth.go`, `packages/server/internal/services/stats.service.go`, `packages/server/internal/api/admin_routes.go`, `packages/server/web/static/index.html`.
...
- **WordService fields are unexported** (`words`, `topicLangs`, `topics`): Cross-package consumers (`internal/api`) access data only through methods (`GetTopics()`, `GetTopicLang()`, `GetRandomWord()`, `SetWords()`). Test helpers inside the `services` package set the fields directly.
...
- **Answer length cap**: Quote answers (the `word` field) must be at most 9 characters. Beyond ~9 letters the fixed `w-12` answer-slot boxes outgrow the `sm:max-w-[430px]` page layout and shrink (`flex-shrink`), making letters cramped. If a new quote's punch word is longer than 9 chars, retarget the `<template>` to a different shorter word in the same sentence instead of removing the quote. `scripts/validate-quotes.mjs` (run via lint-staged on the quotes file) enforces both the 200-char definition cap and this answer cap.
